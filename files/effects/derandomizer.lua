...
if player then
  ...
  local r =  GlobalsGetValue("derandomize_number", Random(1, #ANIMALS))  
  GlobalsSetValue("derandomize_number", r)
  r = tonumber(r)
	for i = 1, #enemies, 1 do
		local enemy = enemies[i]
		if EntityGetName(enemy) ~= "$animal_boss_limbs" and EntityGetName(enemy) ~= "$animal_boss_centipede" and not EntityHasTag( enemy, "NOT_RANDOM_ENEMY" ) then
			eX, eY = EntityGetTransform(enemy)
      local nenemy = ANIMALS[r]
			local new_enemy = EntityLoad(nenemy.path, eX, eY)
			EntityKill(enemy)
      EntityAddTag( new_enemy, "NOT_RANDOM_ENEMY" )
		end
	end  
end